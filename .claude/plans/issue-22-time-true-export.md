...
这些是当前实现的边界，两轮终审记录在案、经权衡未纳入本次范围：

- **预览播放不是导出产物的所见即所得**：扫拨条按弧长匀速推进，只有「跑完一圈的总时长」
  与导出窗口对齐；标签上的时刻按**整条轨迹**插值。导出窗口是子区间时，预览的速度感
  与产物不一致。
- **首尾点缺时间戳时两端有一帧跳变**：`progressAtTime` 在 `tMs <= startMs` / `>= endMs`
  处硬取 0 / 1，而区间内紧邻的插值值是首/末锚点的弧长比例，两者在锚点未覆盖全轨时不等。
- **切换折叠开关会重置已选的起止时刻**：折叠改变时间轴本身，`refreshTimeMode` 随之把
  输入框恢复成完整范围，手选的窗口会丢。
- **sidecar 依赖程序化下载**：长时间导出结束时距用户手势已久，浏览器可能拦下这次下载。
  已单独兜底——MP4 照常落盘，只在成功文案后追加提示。
- **稀疏轨迹（<500 点）有微小弧长偏差**：投影层会对这类轨迹做平滑加密，重采样后的
  弧长比例与原始折线略有出入。骑行 FIT/GPX 普遍数千点，不触发。
- **跨夏令时折返的轨迹**：起止时刻按本地墙钟往返，折返小时内的时刻串有歧义。
